...
Ниже перечислены правила, которые применяются при обработке текста. Порядок: числовое форматирование → пробелы перед единицами → нормализация регистра единиц → глобальные правила → правила конкретного языка.

1 Числовое форматирование (можно отключить)
Отдельная функция форматирует числа, проценты и валюты по языку.
...
- FR: NBSP (`1 234`).
- DE/NL: точка (`1.234`).
- ES: точка только для чисел от 10 000 (например, `1000`, `9999`, но `10.000`).
...
Отдельная функция приводит регистр символов единиц к стандартам ISO/СИ. Применяется только в связках «число + единица».
